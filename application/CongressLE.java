 ...
 */
public class CongressLE extends JPanel {
	...
	public static JFrame demo() {
		...
		try {
			...
			t = new DelimitedTextTableReader().readTable("WHOLifeExpectancy.txt"); 
		} catch ( Exception e ) {
			e.printStackTrace();
			System.exit(1);
		}
		...
		JFrame frame = new JFrame("W H O |  L I F E  E X P E C T A N C Y");
		frame.setContentPane(new CongressLE(t));
		frame.pack();
	...
	//
	private static final String TOTAL_RECEIPTS = "Births attended by skilled health personnel (%)";
	...
	public CongressLE(Table t) {
		...
		receiptsQ.getNumberModel().setValueRange(0,110,0,110);
		...
		//
		DataColorAction color = new DataColorAction(group, "Births attended by skilled health personnel (%)",
