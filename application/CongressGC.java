 ...
 */
public class CongressGC extends JPanel {
	...
	public static JFrame demo() {
		...
		try {
			...
			t = new DelimitedTextTableReader().readTable("WHOGenderComposition.txt"); 
		} catch ( Exception e ) {
			e.printStackTrace();
			System.exit(1);
		}
		...
		JFrame frame = new JFrame("W H O |  G E N D E R  C O M P O S I T I O N");
		frame.setContentPane(new CongressGC(t));
		frame.pack();
	...
	//
	private static final String TOTAL_RECEIPTS = "TeamSize";
	private static final String RECEIPTS = "TeamSize";
	//
	...
	public CongressGC(Table t) {
		...
		vt.addColumn("label", 
				"CONCAT(CAP(Country), ' (', " 
						+ "CAP([NOC]), '-', [Country], "
						+ "') ', '- Total: ', [TeamSize], ', Male: ', [M2012], ', Female: ', [F2012])");
		//
		...
		receiptsQ.getNumberModel().setValueRange(0,500,0,500);
		...
		//
		DataColorAction color = new DataColorAction(group, "TeamSize",
	...
	private class Counter extends GroupAction {
		...
		public void run(double frac) {
			...
			if ( count == 1 ) {
				...
				m_totalStr = NumberFormat.getInstance().format(total) + " person";
			}
